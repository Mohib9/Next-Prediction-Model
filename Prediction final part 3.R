...
model <- readRDS("F:/DS/Course 10 Capstone Project/model.rds")


calculateSimpleGoodTuring <- function(model){
  
  freqTable <- table(model$frequency)
...
predictNGram <- function(resultDF, labelName, sgt, validResultsList, subGram) {
  if(nrow(subGram) > 0 & !(nrow(resultDF) > 0)) {
    #print(labelName)
    subGram$probAdj <- sapply(subGram$frequency, FUN = function(x) sgt$p[sgt$r == x])
...
saveRDS(sgt, "F:/DS/Course 10 Capstone Project/sgt.rds")
