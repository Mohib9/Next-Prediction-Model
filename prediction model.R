...
path <-  ("F:/DS/Course 10 Capstone Project/") 
  
  
sample_size = 0.8
news <- stri_split_lines1(readtext(paste0(path,"en_US.news.txt")))
news <- sample(news, size=length(news)*sample_size)
blogs <- stri_split_lines1(readtext(paste0(path,"en_US.blogs.txt")))
blogs <- sample(blogs, size=length(blogs)*sample_size)
twitter <- stri_split_lines1(readtext(paste0(path,"en_US.twitter.txt")))
